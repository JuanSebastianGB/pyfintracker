...
class TestUpsertRate:
    ...
    def test_upsert_updates_rate(self, connection: Connection) -> None:
        ...
        upsert_rate(connection, r1)
        result = upsert_rate(connection, r2)
    ...
    def test_upsert_returns_db_id(self, connection: Connection) -> None:
        ...
        """
        r = Rate(date=date(2026, 7, 18), from_ccy="USD", to_ccy="COP", rate=Decimal("3255.56"))
        returned = upsert_rate(connection, r)

        assert returned.id is not None
        assert returned.id >= 1

        cached = get_cached_rate(connection, "USD", "COP", date(2026, 7, 18))
...
class TestGetCachedRate:
    ...
    def test_get_cached_rate_hit(self, connection: Connection) -> None:
        """get_cached_rate returns stored Rate."""
        r = Rate(date=date(2026, 7, 18), from_ccy="USD", to_ccy="COP", rate=Decimal("3255.56"))
        upsert_rate(connection, r)
    ...
    def test_get_cached_rate_miss(self, connection: Connection) -> None:
        """get_cached_rate returns None for missing pair."""
        cached = get_cached_rate(connection, "USD", "EUR", date(2026, 7, 18))
        assert cached is None

    def test_inverse_lookup_via_direct_only(self, connection: Connection) -> None:
...
class TestListCachedRates:
    ...
    def test_list_cached_rates_empty(self, connection: Connection) -> None:
        """list_cached_rates returns empty list when no rates."""
        rates = list_cached_rates(connection)
        assert rates == []

    def test_list_cached_rates(self, connection: Connection) -> None:
    ...
    def test_list_cached_rates_since(self, connection: Connection) -> None:
        ...
        rates = list_cached_rates(connection, since=date(2026, 7, 15))
        assert len(rates) == 1
