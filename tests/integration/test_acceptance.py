...
def _init_for_add_flag(tmp_path: Path, cli_runner: CliRunner) -> None:
    ...
    r = cli_runner.invoke(
        app,
        ["account", "new", "Assets:TestChecking", "--currency", "COP"],
        env={"FIN_DB_PATH": str(db_path)},
    )
    assert r.exit_code == 0, r.stdout
    r = cli_runner.invoke(
        app,
        ["account", "new", "Expenses:TestGroceries", "--currency", "COP"],
        env={"FIN_DB_PATH": str(db_path)},
    )
    assert r.exit_code == 0, r.stdout
...
SCENARIOS: list[tuple[Any, ...]] = [
    ...
    pytest.param(
        [
            ...
            "Assets:TestChecking",
            "--to",
            "Expenses:TestGroceries",
            "--amount",
            "50000",
            "--description",
            "Integration test txn",
        ],
        0,
        "Transaction",
        _init_for_add_flag,
